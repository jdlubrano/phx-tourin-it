defmodule TourinItWeb.User.PasskeyLive.Index do
  ...
  def handle_event(
        "passkey_added",
        %{"client_data" => client_data, "attestation_object" => attestation_object},
        socket
      ) do
    passkey_result =
      case Wax.register(
             Base.decode64!(attestation_object),
             client_data,
             socket.assigns.challenge_struct
           ) do
        {:ok, {authenticator_data, _attestation_result}} ->
          ...
          case Accounts.create_user_passkey(passkey_attrs) do
            ...
            {:error, changeset} ->
              error_messages =
                Enum.map(changeset.errors, fn {attr, error_tuple} ->
